...
This note resolves the product question in [issue #3](https://github.com/todorone/gamebuds/issues/3): what Gamebuds must build into its MVP, and what must remain a jurisdiction-specific launch gate, for an English-language iOS and Android game aimed at Players aged 13–30.
...
Gamebuds can preserve its intended low-friction experience, including account-free first play, but the safe MVP is narrower than the conceptual product:
...
| Ephemeral content | Keep ordinary Ephemeral Play Content only while the Game Session is active and delete it immediately when the Game Session ends. Reconnect recovery exists only before that end boundary. Clearly disclose the narrow exception for content attached to a safety report or legal hold. | Storage limitation requires identifiable data to be kept no longer than necessary; transparency must state retention periods or criteria.[^gdpr] | Validate whether any market requires preservation of reported content; do not retain ordinary content “just in case.” |
| Analytics and SDKs | No ads, ATT tracking, AAID, fingerprinting, third-party analytics, screen-recording analytics, heatmaps, or raw UGC in analytics. Use first-party event counts keyed to rotating Game Session identifiers. | Apple requires disclosure of the app and integrated partners' collection, ATT for cross-company tracking, and valid privacy manifests/required-reason declarations; Google makes the developer responsible for SDK data practices and Data Safety accuracy.[^apple-privacy][^apple-att][^apple-sdk][^google-user-data][^google-data-safety] | Review every SDK and processor, device-storage/consent rules, data-transfer mechanism, and privacy label before each release. |
...
## Current regional constraint inventory

No finite note can establish legal clearance for every country. The EEA, UK, and US sections below cover the broad regimes most likely to shape the architecture; this additional inventory captures current non-US regional requirements already concrete enough to affect storefront selection or implementation. It remains a launch checklist, not an exhaustive statement of local law.

| Region | Current primary-source signal | MVP consequence |
| --- | --- | --- |
| Brazil | Google says the Digital Child and Adolescent Statute outlines obligations for apps aimed at or likely accessed by children/adolescents to ingest store age ranges and prohibits loot boxes in those games. Play Age Signals is the current beta integration. Apple likewise exposes Brazil age categories through Declared Age Range when the user or guardian agrees to share them.[^google-country-requirements][^apple-age-regions] | Keep loot boxes excluded; determine the statute's applicability and commencement from the official law, then implement and test both stores' Brazil signals and ClassInd/IARC declarations before allowlisting Brazil. |
| Australia | The OAIC says its Children's Online Privacy Code will be finalised and registered by 10 December 2026. Its current exposure draft covers specified APP entities that provide covered online services likely to be accessed by children, including qualifying apps and games, subject to the draft's scope and exclusions. Apple already blocks unconfirmed minors from downloading 18+ apps in Australia.[^oaic-code][^apple-age-regions] | Determine whether Gamebuds and its operator are within the final Code, update the DPIA/data flows if so, and release only with a classification that admits 13-year-olds. |
| Canada | The Office of the Privacy Commissioner of Canada's current age-assurance guidance is under consultation until 4 August 2026. It says age assurance should be necessary and proportionate, its result must not be reused, and it must not link a person's visits.[^canada-age-assurance] | Keep the minimal age-band design, do not use it as an identifier, and recheck the post-consultation guidance before allowlisting Canada. |
| South Korea | Apple requires a GRAC Rating Classification Number for games meeting its listed conditions before they can be offered in South Korea.[^apple-app-info] | Complete the local classification decision and record any required GRAC number before enabling the storefront. |
| Japan | Google Play says paid apps and apps with in-app purchases may need to display the operator's name, telephone number, and physical address under the Specified Commercial Transactions Act flow.[^google-country-requirements] | Decide whether the solo operator can publish the required business contact details and complete the merchant disclosure before enabling Japanese purchases. |
| China mainland | Apple says games offered on its China mainland storefront must provide the approval number issued by the National Press and Publication Administration.[^apple-app-info] | Keep China mainland disabled unless the approval and all other local operating requirements are obtained. |
| Brazil, Indonesia, Singapore, and Thailand on certified Android devices | Android says that from September 2026 apps must be registered to a verified developer to remain installable on certified devices in these countries, with broader rollout later.[^android-verification] | Complete Android developer identity and app registration before any affected launch; do not treat Play publication alone as the full Android distribution check. |
...
Current platform-signal coverage is partial and must be implemented as a capability, not assumed as a universal answer:

| Signal path | Current availability at this snapshot | Required no-signal behavior |
| --- | --- | --- |
| Apple Declared Age Range | Available on iOS/iPadOS/macOS 26 or later; sharing remains user/guardian controlled, and additional regulatory flags vary by region, including Brazil, Utah, and Louisiana.[^apple-age-range][^apple-age-regions] | If the OS/API is unavailable, the Player declines, or no usable band returns, use the neutral self-declared screen. |
| Google Play Age Signals | Beta. Google currently documents live 0.0.3 signals only for eligible Brazil and Texas users/accounts; availability and returned bands vary, and the API can return no usable signal.[^play-age-signals][^play-age-regions][^google-country-requirements] | Never infer “adult” from missing/unknown. Use the neutral self-declared screen and keep optional collection disabled. |
| Neither platform path usable | Expected on older OS versions, unsupported storefronts, some accounts, and non-store builds. | Use the same neutral self-declared bands, apply high-privacy defaults to every Player, and block under-13 results. Do not collect more identity data merely to replace a missing platform signal. |
...
| --- | --- |
| Private Session | random Game Session ID, hashed/redeemable invitation secret, Game ID/version, coarse timestamps, role, connection state |
...
| Product learning | Game and Game Session events listed in the analytics section, keyed to a rotating Game Session ID |
...
| Game Session state and Session Identity | While the Game Session is active | Automatic hard delete when it ends; reconnect is available only before that boundary |
| Ordinary Ephemeral Play Content | While the Game Session is active | Automatic hard delete when it ends; never copy to analytics or backups |
...
The Host can end a Game Session explicitly. The service also ends it automatically after 30 minutes with no connected Player and applies a hard two-hour lifetime from creation; reconnect recovery is possible only while it remains active. Ending triggers deletion of Game Session state, Session Identity, and ordinary Ephemeral Play Content. A report submitted before the end may copy only its minimum evidence into the separate safety-report store.
...
- Store ownership and Gamebuds identity are separate. Free play never requires an account. A store purchase can be restored on the purchasing platform without forcing unrelated profile collection. Cross-platform entitlement portability may require an optional Player Account, and the purchase screen must say so before payment.
- Use short-lived access tokens, rotating refresh tokens, provider-token verification, rate limiting, replay protection, device and access-token revocation, and reauthentication for deletion or sensitive account changes. Do not use Session Invitations as account authentication.
...
1. Limit creation to structured prompt answers with visible length limits and drawings made in the Gamebuds canvas. Do not allow pasted links, images, files, camera/photo uploads, audio, video, arbitrary chat, or content import.
...
3. Run proportionate pre-distribution controls for text and drawings, plus rate/size limits. A filter is not the whole moderation system: a Player must be able to report content or a Player from the active Game screen and the Game Session summary, and the service must act on reports.[^apple-review][^google-ugc]
4. Give every Player **mute/hide**, **block**, **leave**, and **report**. Give the Host **kick** and **revoke invitation**. A block prevents the blocked identity from joining the blocker's future Game Sessions when the service can recognise the identity; for account-free Players, maintain a privacy-preserving local/device block and enforce removal from the active Game Session.
5. Make Session Invitation tokens high entropy, short-lived, revocable, limited to one Game Session, and excluded from logs, analytics, indexing, previews, and Result Cards. Joining always shows the Game and Host/Play Group context before entry; never auto-enrol from a scanned link.
...
- Maintain an incident playbook covering account and Game Session containment, Player notification, evidence preservation, platform escalation, qualified legal review, and regulator/law-enforcement reporting. Do not improvise automated scanning or external reporting without assessing accuracy, privacy, security, and local duties.
...
- Game Session content deletion explanation and safety-report exception;
- optional account access/export, correction, deletion, provider unlinking, and token/device revocation;
- a way to request deletion of account-free Game Session/support data using the minimum verification possible;
...
The following product architecture does not depend on finding a single global age of consent: 13+ positioning; under-13 block; high-privacy defaults for all; exact-birth-date avoidance; no ads/tracking/AAID/fingerprinting; no contacts/location/camera/audio/imported media; account-free first play; optional minimal account; finite Private Sessions; constrained Ephemeral Play Content; report/block/kick/leave; short retention; first-party Game Session analytics; native store billing; no manipulative monetisation; accurate store declarations; and country allowlisting.
...
[^google-country-requirements]: Google Play Console Help, [Requirements for distributing apps in specific countries/regions](https://support.google.com/googleplay/android-developer/answer/6223646?hl=en).
[^android-verification]: Android Developer Console Help, [Understanding Android developer verification](https://support.google.com/android-developer-console/answer/16561738?hl=en), including the September 2026 first-country rollout.
...
[^oaic-code]: Office of the Australian Information Commissioner, [Children's Online Privacy Code](https://www.oaic.gov.au/privacy/privacy-registers/privacy-codes/childrens-online-privacy-code), including its 10 December 2026 finalisation deadline and current exposure-draft status.
[^canada-age-assurance]: Office of the Privacy Commissioner of Canada, [Assessing whether and how to use age assurance](https://www.priv.gc.ca/en/privacy-topics/age-assurance/aa-gd-web/), current consultation closing 4 August 2026.
